code = '''be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
real_code = '''caebgd dagc eabgd aebfgc fbdacge edg dg dbgcef eabfd cgeba | gbcdae dagc acgd gd
...
def get_digit(line):
	...
	def sort_by_length(e):
		return len(e)
	inp_digits.sort(key=sort_by_length)

	# 1 inp_digits[0]
	for index, val in enumerate(possible_mapping):
		if index == 2 or index == 5:
			possible_mapping[index] = [inp_digits[0][0], inp_digits[0][1]]
		else:
			val.remove(inp_digits[0][0])
			val.remove(inp_digits[0][1])

	# 7 inp_digits[1]
	for letter in inp_digits[1]:
		if letter not in possible_mapping[2]:
			for index, val in enumerate(possible_mapping):
				if letter in val:
					val.remove(letter)
			possible_mapping[0] = [letter]
			real_mapping[0] = letter

	# 4 inp_digits[2]
	possible_mapping[1] = []
	possible_mapping[3] = []
	for letter in inp_digits[2]:
		if letter not in possible_mapping[2]:
			for index, val in enumerate(possible_mapping):
				if index not in [1,3]:
					if letter in val:
	...
	for i in range(3):
		letters_in_inp = True
		for letter in possible_mapping[1]:
			if letter not in inp_digits[i + 3]:
				letters_in_inp = False
		if letters_in_inp: # inp_digits[i + 3] is #5
			five_index = i + 3
			[first_letter, second_letter] = possible_mapping[2]
			if first_letter not in inp_digits[i + 3]: # letter is real_mapping[2]
				real_mapping[2] = first_letter
				real_mapping[5] = second_letter
				possible_mapping[2] = [first_letter]
				possible_mapping[5] = [second_letter]
			else:
				real_mapping[2] = second_letter
				real_mapping[5] = first_letter
				possible_mapping[2] = [second_letter]
				possible_mapping[5] = [first_letter]

	# 3
	three_index = None
	for i in range(3):
		if real_mapping[2] in inp_digits[i + 3] and real_mapping[5] in inp_digits[i + 3]:
			three_index = i + 3
			# this is three
			for letter in inp_digits[i + 3]:
				if letter not in [real_mapping[0], real_mapping[2], real_mapping[5]]:
					if letter in possible_mapping[3]:
						possible_mapping[1].remove(letter)
						real_mapping[1] = possible_mapping[1][0]
						possible_mapping[3] = [letter]
						real_mapping[3] = letter
					if letter in possible_mapping[6]:
						real_mapping[6] = letter
						possible_mapping[4].remove(letter)
						real_mapping[4] = possible_mapping[4][0]
	...
	for i, letter in enumerate(real_mapping):
		reversed_mapping[letter] = i

	str_num = ''
	for digit in outp.split():
		indexes = []
		for letter in digit:
			indexes.append(reversed_mapping[letter])
		indexes.sort()
		str_num += str(nums.index(indexes))
...
def main(code):
	...
	for line in lines:
		# count += part1(line)
		count += get_digit(line)
	return count
		

# print(main(code))
print(main(real_code))