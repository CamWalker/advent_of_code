...
def get_prime_bit(bit_map, index):
	if bit_map[index]['0'] > bit_map[index]['1']:
		return '0'
	return '1'

def get_unprime_bit(bit_map, index):
	if bit_map[index]['0'] <= bit_map[index]['1']:
		return '0'
	return '1'
...
def last_dig_rem(index, values, func):
	...
	return binaryToDecimal(filtered_vals[0])
...
def calculate_power_cons(input):
	...
	for index in bit_map:
		gamma += get_prime_bit(bit_map, index)
		epsilon += get_unprime_bit(bit_map, index)
