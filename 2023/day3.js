...
class Num {
	constructor(startX, endX, Y, number) {
		this.startX = startX;
		this.endX = endX;
		this.Y = Y;
		this.number = number;
			...
			for (let i = 0; i < _row.length; i++) {
				...
				if (/\d/.test(cell)) {
					if (!foundNum) {
						...
						currentNum.endX = i;
						currentNum.number += cell;
					}
					foundNum = true;
				} else {
					foundNum = false;
					if (currentNum) {
						numbers.push(new Num(currentNum.startX, currentNum.endX, currentNum.Y, currentNum.number))
						currentNum = null;
					}
				}
			}
	...
	return numbers
		.filter(num => {
			for (let i = 0; i < symbols.length; i++) {
				const [x, y] = symbols[i];
				if (num.isAdjacent(x, y)){
					return true;
				}
			}
			return false;
		})
		.map(num => parseInt(num.number, 10))
		.sum()
}
...
function part2(inp) {
	...
	return symbols
		.filter(([x, y]) => {
			const arr = numbers.filter(num => num.isAdjacent(x, y))
			return arr.length === 2;
		})
		.map(([x, y]) => numbers.filter(num => num.isAdjacent(x, y)).map(num => parseInt(num.number, 10)).product())
		.sum()
}
