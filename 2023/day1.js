...
function execute2(inp) {
	let rows = inp.split('\n');
	rows = rows.map(row => {
		row = row.replaceAll(/\D+/g, '');
	...
	rows = rows
		.map(row => {
			for (let i = 0; i < row.length - 2; i++) {
				if (/\d/.test(row[i])) return row;
				for (let j = 3; j < 6; j++) {
					const length = j;
					if (nums[row.substring(i, i + length)]) {
						row = nums[row.substring(i, i + length)] + row;
						return row;
					}
				}
			}
		...
		.map(row => {
			for (let i = row.length - 1; i >= 0; i--) {
				...
				for (let j = 3; j < 6; j++) {
					const length = j;
					if (nums[row.substring(i, i + length)]) {
						row = row + nums[row.substring(i, i + length)];
						return row;
					}
				}
			}
